...
function initDecrypt() {
	var encrypted = null;
	if(selectedTitle.parent().parent().parent().prop("id") == "query") {
		if(isEncrypted) {
	...
	if(isMarked && selectedTitle.parent().parent().parent().prop("id") == "query") {
		mark();
...
function decryptChild() {
	$("table#query_child td.encrypted_title").each(function() {
		if($(this).text() != "") {
...
function decryptHistory() {
	$("table#query_history td.encrypted_title").each(function() {
		if($(this).text() != "") {
			var encrypted = $(this).text();
...
//$(document).ready(function() {
	if(isEncrypted || isChildEncrypted || isHistoryEncrypted) {
		selectedTitle = $("table#query td.title");
		selectedContents = $("table#query td.contents");
		selectedContents2 = $("table#query td.contents2");
	...
	$("table#query_history td.title").each(function() {
		$(this).click(function () {
			if(
				(
...
function diff(target) {
	...
	var other = $("table#query td.contents").text();
	$("pre#display").empty();
...
function autoSave(target) {
	var formData = null;
	var url = null;
	if(target.parent().find("td.query_history_id").text() == "") {
		return;
	}
	formData = new FormData();
	formData.append("query_history_id", target.parent().find("td.query_history_id").text());
